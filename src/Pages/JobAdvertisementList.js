...
import { Divider, Grid, Image, Segment, Card, List, Button, Item, Label, Header, Icon, Menu, Input, Sticky } from 'semantic-ui-react';
import AddAdvertisementModal from './AddAdvertisementModal';
import { getCityList } from '../services/CityService';
import { getJobPositons } from '../services/JobPositionService';
function JobAdvertisementList() {
    const [cities, setCities] = useState([]);
    const [jobPositons, setJobPositions] = useState([]);
    useEffect(async () => {
        let cityList = await getCityList();
         let jobPostionsList = await getJobPositons(true);
        setCities(cityList)
         setJobPositions(jobPostionsList);
    }, [])
    return (
        <>
            <Grid>
                <Grid.Row>
                    <Grid.Column width={3}>
                        ...
                        <Menu vertical>
                            <Menu.Item
                                name='inbox' >
                                <Label color='teal'>1</Label> Inbox</Menu.Item>
                            <Menu.Item
                                name='spam'>
                                <Label>51</Label> Spam
                            </Menu.Item>
                    ...
                    <Grid.Column width={13} style={{ height: '100%' }}>
                        <Segment style={{ height: 'calc(100vh - 94px)' }}  >
                            <Sticky>
                                <Grid>
                                    <Grid.Column width={6} className="listGrid" style={{ height: 'calc(100vh - 94px)', overflowY: 'scroll' }}   >
                                        <AddAdvertisementModal jobPositons={jobPositons} cities={cities} />
                                        <Divider />
                                        <List>
                                            <List.Item active>
                                                <Card fluid className="dashBoardContainer" color="green">
                                                    <Card.Content>
                                                        <Image
                                                            circular
                                                            floated='right'
                                                        ...
                                                        />
                                                        <Card.Header><a>Frontend Developer</a></Card.Header>
                                                        <Card.Meta>DetaySoft</Card.Meta>
                                                        <Card.Meta>İstanbul,Sarıyer</Card.Meta>
                                                        <Card.Description>
                                                            <Label as='a' image>
                                                                Tam Zamanlı
                                                                <Label.Detail>19 gün önce</Label.Detail>
                                                            </Label>
                                                        </Card.Description>
                                                    </Card.Content>
                                                </Card>
                                    ...
                                    <Grid.Column width={10} style={{ height: '60%', overflowY: 'scroll' }}>
